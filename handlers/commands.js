...
/**
 * This helper function limits the amount of chars in a string to max 23 and adds "..." if shortened.
...
function trimEventName(eventName){
  if(eventName.length >= 24){
    eventName = eventName.trim().substring(0, 20) + "...";
  }
  return eventName;
}
    ...
    cal.Events.list(calendarID, params).then((json) => {
      for (let day = 0; day < 7; day++) {
        ...
        for (let i = 0; i < json.length; i++) {
          ...
          let type = classifyEventMatch(dayMap[day], eStartDate, eEndDate);
          if (type != eventType.NOMATCH) {
            matches.push({
              id: json[i].id,
    ...
    if (calendar[key].length === 0) {
      ...
      for (let m = 0; m < calendar[key].length; m++) {
        ...
        if (Object.keys(calendar[key][m].start).includes("date")) {
          ...
          tempString["All Day"] = trimEventName(calendar[key][m].summary);
          ...
          tempString[tempStringKey] = trimEventName(calendar[key][m].summary);
          sendString += columnify(tempString, options) + "\n";
        }
      }
      sendString += "```";
