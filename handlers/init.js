...
exports.run = async function(message) {
  ...
  if (['setup','init'].includes(cmd) || mentioned(message, ['setup','init'])) {
...
``!id`` <calendarID> to enter your google calendar ID\n``!tz`` <timezone> to enter your timezone, in the form ``GMT+10:00`` (currently must use GMT and maintain full form)');
  ...
  if (cmd === 'id' || mentioned (message, 'id')) {
      ...
      collector.on('collect', (m) => {
        if(m.content.toLowerCase() === 'y' || m.content.toLowerCase() === 'yes') {
          ...
          fs.writeFile('./stores/guilddb.json', JSON.stringify(guilddb, '','\t'), (err) => {
            if (err)
              return console.log(Date() + ' write CalendarID error: ' + err);
          });
        } else {
    ...
    if (cmd === 'tz' || mentioned(message, 'tz')) {
...
//helper functions
//Should import this from Commands.js
